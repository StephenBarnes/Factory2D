...
* DEFER For community puzzles, organize them automatically by their set of allowed components - they should be unlocked after the first built-in puzzle which includes all of those components among its recursive prerequisites. Construct the set of "components that have been introduced so far" for each built-in puzzle by taking union of those sets for each prereq.
...
General:
* Allow configuring some components using a modal box. Modal is opened when placing the block (for some of them, depending on a flag) and by pressing E while mouse is over them. Show control prompt in the tile inspector panel.
...
* Add ROM component: +1/-1 on one side moves cursor, other sides output the stored value. E-key configuration modal allows setting ROM size (width and height) and value in each cell - display the values as a grid of blue/red/black cells for +1/-1/0 values. Left-click cycles black -> blue -> red -> blue -> red; right-click sets to black. The 2D dimensions are just for display, it's read as a 1-dimensional array of ternary values.
...
Circuit network:
* Add delays, with configuration panel to set the delay, up to some value like 27. Render with a grid of dots in the center, showing charges moving through. The delay should internally be like a ring buffer - each tick, we do something like `cursor[t+1] = (cursor[t] + 1) mod length`, `out[t+1] := data[t][cursor[t]]`, and `data[t+1][cursor[t]] = in`.
* Add shift+scrollwheel to configure delays, counters, and later any other components where configuration is a single number.
