...
* Add a dispenser component that dispenses a copy of the welded body behind it, creating the duplicate in front of it, when it receives a charge on the side. Only dispense if the created copy would fit (would only occupy tiles that are currently empty).
