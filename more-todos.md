...
* Modify the charge counter to decrement its count when it receives a -1 charge, increment on +1. Make it output +1 when it moves from the max value around to zero, and output -1 when it decrements from zero, which should set it to the max value.
...
* Modify the rendering for the charge counter and the delay, to put two arrows `>` on input and output sides, since otherwise orientation isn't visible. Also modify ROM rendering to show the input side; other 3 sides are output so don't need to be marked. Also for the ROM, render the grid on the block using dark purple for zero values instead of pure black. For all three of these, make their connected wires extend less far into the interior of the block, since it can overlap with their displays currently. For all three, also modify the description to mention pressing E to configure them.
