...
* Palette hover uses `TileDefinition.palette.extendedDescription` paragraphs alongside the short description in a wider inspector. The combiner reference includes a signed, color-coded truth table for all three inputs (left/right grouped by sum). Board hover retains only the short description and live component properties; tool help remains separate.
