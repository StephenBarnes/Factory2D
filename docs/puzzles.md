...
* [Steelworks](../src/game/puzzles/steelworks.json) (Intermediate): smelt six supplied iron blocks and deliver six separate steel blocks. Eight active furnace ticks require wood on both lateral sides of the iron; all touching wood burns on completion. A narrow editable strip above the supply permits overhead fuel loading, while the lower workshop supports parallel hearths or a shared processing line. Sealed examples on the right contrast correct two-sided fuel with an incomplete hearth. Iron, steel, platforms, duplicators, thrusters, and force projectors are unavailable in the palette. The reference solution uses slider-supported hearths and burning wooden floors to release steel onto conveyors; it completes in 30 cycles.
