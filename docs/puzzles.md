Current shipped catalog: **27 puzzles — 7 tutorials and 20 non-tutorials**. Each entry below links to its definition in `src/game/puzzles/`; names, groups, and ordering follow those files.
...
* Bell blocks, mallets, and resonators.
...
* [Copperworks](../src/game/puzzles/copperworks.json) (Intermediate): smelt a finite supply of six copper ore blocks beside wood, then deliver all six copper blocks. Requires wood adjacent to the copper ore to smelt. Reference solution feeds the 3x2 copper ore to a row of 6 furnaces with wood below; they finish smelting in the same tick and then fall once the wood burns.
* [Tinworks](../src/game/puzzles/tinworks.json) (Intermediate): smelt 5 tin ore blocks, which require adjacent lateral wood. Previous solution doesn't work due to gravity / middle furnaces needing support.
* [Steelworks](../src/game/puzzles/steelworks.json) (Intermediate): smelt 3 supplied iron ore blocks and deliver 3 steel blocks.
...
* Puzzles that use an indestructible airlock: once a button is triggered, the near-editable-region door shuts, then the further door opens; it cannot be cycled back, so it's one-way. They have to e.g. use unlimited supply of loose conveyor blocks, fixed-charge blocks, etc. to assemble vehicles indefinitely, sending each one through the airlock to a course where it has to drive up a hill to deliver a gemstone. Use block counters to enforce that the airlock only allows one gemstone through at a time.
* Puzzle based on routing two or more streams of inputs to outputs in a small region, requiring crossings.
* Once we have molten metals, add puzzles based on molds. For example molding a gold ring by pouring molten gold into a mold which is made of two half-ring pieces, which can then separate to drop the finished ring.
* Puzzles that have different small chambers, connected by gate blocks that can transfer bodies between them. So each chamber has to make some intermediate product or implement one stage of production, because there's not enough space to do more.
* Puzzles similar to current manufacturing or extraction puzzles, but with multiple test cases with slightly different spacing, initial phase, position/orientation, etc., so there's less trial-and-error and solutions must be more general.

## Additional candidates: existing mechanics

### Assembly order and material economy

* **The last jewel** (Intermediate): assemble a hollow iron crown with a ruby in its central cavity. The ruby must be welded to the inside of the crown, so the result is one deliverable body. Leave too little clearance to insert it after closing the frame. The interesting decision is where to leave an opening and how to close it without trapping the assembly machinery; ban swappers and flippers in the initial version.
* **Left boot, right boot** (Intermediate): manufacture equal numbers of two asymmetric, mixed-material boot shapes which are reflections but not rotations of each other. Give only one orientation of the stock. Without flippers, the player chooses between separate assembly lines and a shared line that dismantles and rebuilds the asymmetric part. Separate delivery boxes check each handed product.
* **No scrap left behind** (Hard): supply a finite 4x4 iron slab and require three L tetrominoes and two dominoes, using all sixteen blocks. Drilling is available but destroys material needed for victory; splitting, rearranging, and welding must do the real work. Start with a generous workshop, then try a version where offcuts quickly obstruct the next cut.
* **Foundry scaffolding** (Intermediate): build a stepped iron arch using temporary wooden supports, then deliver the iron body with no wood attached. Furnaces can burn away the supports, but the last support must survive until the iron is self-supporting as a welded body. An alternative mechanical jig should remain possible; burning scaffolding trades reusable machinery for consumables.
* **Return the jig** (Hard): supply one special shaping fixture and enough loose material for several products, but no way to copy the fixture. Each product must leave without it, and the original fixture must finally reach a separate receiver. This makes temporary welds, release order, and fixture recovery part of the job rather than letting the player consume the tooling in the first product.
* **Commissioning day** (Hard): assemble and deliver a small working conveyor cart from loose conveyors, stone, and circuit components. A sealed test track must prove that the cart travels with the supplied cargo before accepting it; a shape match alone cannot validate circuit configuration or behavior. Keep the track outside the editable region and check cargo checkpoints plus final delivery, so a factory-mounted pusher cannot substitute for an operational cart.

### Transport, buffering, and reusable carriers

* **Passing place** (Intermediate): two differently marked loads enter opposite ends of a one-cell-high horizontal tunnel with one taller passing bay. Deliver each at the other end without cutting either load. The limited build regions force the machine to park one load out of the other's path; multiple arrival orders test whether it actually manages right of way.
* **Foundry crossroads** (Hard): two streams of multi-block products must cross inside a compact shared chamber and leave through opposite exits. An indestructible perimeter prevents routing around the intersection. Start with generous gaps, then introduce simultaneous arrivals and short bursts: holding one stream, alternating batches, and moving whole carriers give different throughput/footprint tradeoffs.
* **The only pallet** (Intermediate): several loose glass blocks must descend a shaft to a receiver, but only one supplied iron pallet is available for temporary welding. No other weldable support material is placeable along the shaft. The player must retrieve and reuse the pallet, or build a genuinely safe series of short drops; unprotected glass cannot simply fall the full distance.
* **The impatient furnace** (Hard): a fixed supply sends irregular batches toward one narrow processing station. Every supplied ore block must be smelted and delivered, so overflow cannot be discarded. Provide enough total buffer space but not enough to give each input its own processing line. Test bursts separated by quiet periods, with a sustainable average feed rate; the challenge is buffering, not an impossible sustained throughput requirement.
* **Empty wagons home** (Hard): two distinct reusable carriers bring loads from different shelves to one unloading dock. Deliver all cargo and return both empty carriers to their home bays. Make the last trip count too: victory requires the return-state body comparers as well as the cargo counters. This adds a complete service cycle to the existing one-way vehicle puzzles.

### Mining, salvage, and maintenance

* **Magnetic picking** (Easy–Intermediate): a mixed stream of separate iron blocks and nonmagnetic gemstones must be split into two receivers. Give ordinary magnets, conveyors, and pistons, but no block-type comparers. A magnetic pickup can select the iron, yet releasing it is a second mechanical problem because the current magnet has no off switch. Vary the order and include adjacent iron blocks.
* **Follow the seam** (Hard): launch a miner from one small build bay into a horizontal stone seam containing occasional rubies, then return every ruby to the entrance. Different cases move the gems along the same bounded seam. Give enough sensing to stop the drill before it destroys a gem; transport of spoil, access to the next face, and the return journey distinguish this from a stationary geode splitter.
* **The roof pays for itself** (Hard): ore is embedded in load-bearing ribs beneath a large stone roof. Mine and deliver the ore while preserving specified roof markers at their original positions. Protected comparers check the markers continuously and latch failure if one moves. Players can replace supports, leave temporary pillars, or remove ribs in a safe order; no dwarf block is needed for this version.
* **Maintenance hatch** (Intermediate): a supplied welded machine has one wrong material block in a recessed socket. Replace it with the correct block and deliver the repaired body, preserving the surrounding valuable parts. Provide swappers and splitters, but no duplication or replacement stock for the shell. Compare direct cell replacement with partial disassembly; a harder version has several defects in different test cases.

### Mechanical measurement and sound

* **The inspector's hammer** (Intermediate): sort supplied stone bars of three different lengths into separate receivers, without block/body comparers or beam sensors. Mallets can strike the bars; resonators tuned to the three body sizes identify them. The sorting fixture must not weld itself to the bar while measuring, or it changes the pitch. Include touching but unwelded neighboring bars to make isolation useful.
* **One bell, many notes** (Hard): ring a supplied bell in a requested sequence of pitches by attaching and removing ballast blocks. Only one bell is available, and horizontal motion rings it, so retuning and repositioning can accidentally produce extra notes. Choose two pitches for the first version and check their order with protected resonators and an ignore-zero sequence checker; a wider musical vocabulary needs an explicitly designed multi-note judge.

## Infinifactory-inspired candidates

Borrow the production constraint, not just a flattened silhouette. In 3D, a line can often pass over another in a separate depth layer; in our side view, crossings, access to an interior, and assembly order can be the main challenge.

Sources: Zachtronics' [Production Zone 1 update](https://www.zachtronics.com/infiniupdate-2/) describes continuous extruded inputs and a factory-building sandbox; its [Atropos Station / Homeward Fleet update](https://www.zachtronics.com/infiniupdate-3/) describes complex inputs that require processing, massive products, and face painting. The following are proposed adaptations of those mechanics/themes, not claims that these exact puzzles appear in Infinifactory.

* **Cut to length** (Intermediate; existing mechanics with a supplied feed rig): a protected factory advances a continuously welded iron strip into the workshop. Cut and deliver alternating short and long bars without allowing the growing strip to jam the inlet. A fixed duplicator, welder, and transport rig could supply the stock; prototype that rig before deciding an extruder block is needed. Unlike Iron plates, the input is already joined and the central problem is choosing when to separate it.
* **Two products, one carcass** (Hard; existing mechanics): supply large mixed-material elven machines. Their useful subassemblies must become two different products, with both outputs required in proportion to the number of inputs. Arrange the feedstock so destroying the inconvenient middle also destroys something needed by the other product. This develops the existing reworking ideas into a coupled production line, rather than one product plus disposable waste.
* **The longship yard** (Hard–Expert; existing mechanics): assemble one very long dwarven landship from repeated hull sections, with a distinct prow, stern, and cabin. The completed ship is wider than the editable assembly bay, but fits in a protected launch corridor, so it must advance as sections are attached. Keep the number of module types small: the difficulty should be moving and joining a growing body, not placing hundreds of unrelated welders.
* **Heraldic shields** (Intermediate–Hard; new face-painting block and matching support): manufacture asymmetric shields with different colors on their four cardinal edges. A painter coats only the facing edge, and later paint overwrites earlier paint. Rotation and masking would matter in 2D without needing six 3D faces. Delivery/body comparison, transforms, and serialization would all need to preserve and check edge colors; substituting differently colored gemstone blocks would be a cheaper but different puzzle.

## Candidates for planned or new mechanics

Dependencies below refer to [near-term components](todos.md#new-non-circuit-components) and [deferred component ideas](todos-deferred.md#new-non-circuit-components). These are reasons to explore those mechanics, not commitments to implement them. Bells, resonators, and fire already exist; directional gravity, physical nested boxes, tree growth, and crushing-sensitive dwarves do not.

* **Gravity courier** (Intermediate; planned gravity stone): carry a unique gem around a rectangular course by turning the courier so its gravity points toward the next wall. Keep the gem attached and require the courier to return to its starting orientation. Without thrusters or freely placeable extra gravity stones, junctions become questions of how to brace, turn, and launch the same body.
* **Ceiling foundry** (Hard; planned gravity stone): use one reusable gravity stone to move a batch of material upward to ceiling-mounted processing machinery, then bring the processed stock back down. The stone must be detached before final delivery and reused for later batches. This explores changing an entire body's gravity, not a global gravity toggle; mixed loose and welded loads should behave differently.
* **Strike while it's braced** (Intermediate; deferred press/crusher): turn plain stock into rivets using a press recipe, then attach them to a frame. Design the recipe so an unbraced workpiece is pushed away while a properly backed workpiece is transformed. Feeding, backing, and ejecting share the same space; simply pulsing the press cannot complete the job. The exact recipe and whether pressing consumes or preserves the input must be defined first.
* **Magnetic handoff** (Intermediate; deferred electromagnet): move iron loads across a gap using two independently controlled magnetic carriers. One must release while the other takes over, without dropping the load or bringing the carriers together. This complements Magnetic picking: switching and polarity become the new decisions. Prototype with the eventual attraction/contact rules rather than assuming magnets act at unlimited range.
* **A factory in a lunchbox** (Hard; deferred physical nested box): process bulky inputs inside one movable miniature workshop, then carry the box to a different docking station to emit the products. Its inlet and outlet cannot both reach their external stations at once. Start with one nesting level and define whether whole welded bodies can cross a port; the current rune array's circuit ports do not support physical transfer.
* **Coppice keeper** (Intermediate–Hard; deferred leaves and deterministic tree growth): harvest a quota of wood from an awkward stepped hillside while leaving each marked root and a viable growing branch intact. Require a second harvest after regrowth, so clear-cutting the starting trees cannot win. Different deterministic growth cases reward reach and adaptability; the growth rules and a protected way to judge continued viability are prerequisites.
