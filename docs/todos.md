...
* Make middle-click on the game-canvas, outside the actual grid, count the same as middle-click on empty cells of the game grid (which picks the previously-selected block).
* For the sandbox, add buttons on the sides of the block grid, or buttons in the puzzle properties modal, to add rows or columns on the top/bottom/left/right sides of the canvas. Make these shift all blocks correctly, and shift the player-modifiable region. (Currently just incrementing width/height is fine when expanding on the right or bottom, but expanding top/left requires moving everything and adjusting the player-modifiable regions.) The buttons on the sides of the grid would be ideal / better than buttons in puzzle-properties modal, but uncertain if they may be difficult to implement properly.
