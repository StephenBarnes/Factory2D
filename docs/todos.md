...
* Bug: see `temp/piston-retract-push-bug.json`. The piston tries to extend and retract repeatedly. Its head is welded to a cup, which contains an isolated 1-block body. The piston can extend successfully, pushing that 1-block body, but it doesn't retract. Removing the 1-block body lets it retract. We should instead allow it to retract, pushing the 1-block body right.
...
* Add a volume slider in the settings menu. The `tone` function in `src/ui/workshop-sounds.ts:115` has a volume argument, may just need to multiply by the volume set in the settings.
