...
* Currently when e.g. a combiner's output port is connected to a conduit, the wire extends deeper into the combiner block than the arrow. Same issue with many other blocks that have the same standard side arrows. Modify them to make the part of the wire that visually extends into the block shorter, maybe similar to the ROM rune block. It looks like for input sides / arrow pointing into the block, it's fine currently; but for output sides, the extension into the block should be shorter. Currently the combiner's output line is too long, and the charge counter's is also too long (but by a shorter margin) while its input is perfect already. Similar for all other blocks with these output arrows: combiner, multiplier, subtractor, equality, min/max, delay.
* Add an output marking on the rectifier block, similar to those currently on combiner, multiplier, etc.
