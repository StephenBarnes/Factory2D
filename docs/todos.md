...
* Modify smelting-welding behavior of sand/glass so that adjacent sand blocks smelted on the same tick are also welded. (Currently smelting sand to glass will auto-weld to neighboring glass, but smelting multiple sand blocks that reach completion on the same tick will not weld them, which is a confusing special case.)
