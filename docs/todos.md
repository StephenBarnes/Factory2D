...
* Modify furnace adjacency rules. We have one rule already - smelting copper ore requires a wood block adjacent to the copper ore, not consumed.
	* Modify this so that copper smelting changes the wood catalyst into a fire block, consuming it. Ideally expand this in a way that allows defining rules like this (transforming the catalyst, or not) for future smelting recipes.
	* Add more complex patterns of requirements, e.g. the catalyst must be on a side not opposite the furnace (so furnace-ore-wood must form 90-degree bend), or it must be on both sides not opposite the furnace, or all 3 sides of the ore except the furnace. Add this for other ores, since copper ore already has behavior; e.g. for metals like steel, galvanized iron, mithril, or others; would need to add blocks for those.
