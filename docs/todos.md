...
* Modify saved puzzle solutions. Currently if the player tests a solution, and succeeds, and presses "back to briefing", and then tries to edit the solution again, it starts in the finished reset state. So they have to click reset before doing anything. We should instead store the state before they started testing. If they duplicate their solution and then click edit on the duplicate, it seems to also sometimes start in the already-run state that requires a reset.
