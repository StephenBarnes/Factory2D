...
* EASY Check whether the `shadow` field in tile.ts is currently used anywhere. It seems to be dead. If so, remove it.
* Currently when e.g. a combiner's output port is connected to a conduit, the wire extends deeper into the combiner block than the arrow. Same issue with many other blocks that have the same standard side arrows. Modify them to make the part of the wire that visually extends into the block shorter, maybe similar to the ROM rune block.
* Modify the side input/output arrows/carets of the delay, charge counter, and ROM rune blocks - currently they're drawn a fixed white color. Instead draw them with the color of charge they have there, similar to other blocks (like the combiner). (The white arrow/caret on the ROM grapher can stay, since it doesn't form circuit connections.)
* Add the same input arrows on the duplicator and rotator - because they all take circuit inputs on some sides. (Not necessary for the furnace or the piston, because they have input/output on multiple sides and we don't want to clutter them with too many arrows.)
* Make the glass block look more like transparent glass. Maybe by making it darker in the middle and brighter on the rim to simulate Fresnel? We might not need actual rendered transparency. Maybe just don't draw the interior solid fill, but draw one white line in top-left to look like specular.
