...
* Check for any potential bugs caused by listening only to mouse-up and mouse-down events, and assuming the mouse button is held down until a mouse-up is received. Can cause accidental deletion or placing of tiles if the mouse-up event is hidden by other window events. Currently seems to occur if I use shift + right-click to open context menu (on Firefox), then close the menu - mouseover deletes blocks until I right-click again.
