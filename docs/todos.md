...
* Add a rotator component. It has a circuit input on one back side. It faces in a specific direction, but stores an internal direction that's either forward, left, or right, indicated on the rendered block; cannot face back to the circuit input. Signals of +1 and -1 rotate that internal direction by 90 degrees at a time. Each time it rotates, it also attaches to the block in that direction, and then rotates that block's entire body to keep that edge against its new internal direction. If the body can't be moved like that due to collisions, instead prevent rotation. One problem: some blocks don't do the same thing when rotated, specifically ROM blocks; we'll handle that as a separate task below. A different bigger problem: if we have a welded box, with unwelded blocks inside it, we probably want those contained blocks to rotate with the box - not have rotation teleport the box away, releasing the contents. Similarly if a U-shaped body is hooked into an upside-down T-shaped body, rotating the U should probably attempt to rotate the T with it. So we need to define what it means for a body rotating in a specific direction to "contain" or be obstructed by other blocks, and then attempt to rotate all of those with the containing body. This probably also creates some surprising behavior, e.g. you could put a rotator inside a box and rotate one block in the corner to rotate the containing box. Later, also animate these rotations.
