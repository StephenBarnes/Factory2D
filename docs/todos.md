...
* For the rotator component, an input circuit connection currently draws the circuit line to the center of the block. Rather make it draw like other components like the delay block, with the circuit trace terminating a short distance inside the block to match with the input arrow.
