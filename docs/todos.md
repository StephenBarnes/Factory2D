...
* Modify the charge sensor rune to allow sensing at a distance. Walk from the rune to the edge of the board, stopping at the first non-empty block, and attempt to read its charge on the near side; if it's a block with no circuit interaction (e.g. stone) then read charge zero. (This is useful because it allows us to give circuit inputs in a puzzle, without forcing the player's design's footprint to be large enough to touch the circuit input ports.)
