...
* For signal traces drawn in the signals panel, allow click and drag to reorder them. Probably store ordering on the signal monitor and ROM-grapher components, but hide that number - don't add a box to edit the number directly in the config modal. Only allow reordering inside each category.
...
* DEFER On mouseover on the signal panel, we highlight the relevant grapher or monitor that produces that trace, but only if it's in the current board, not if it's nested inside a rune array. We should instead highlight the rune array containing it, if possible. (It's also possible the view is currently inside a rune array, so the monitor/grapher is outside it, in which case we can't highlight it, or could e.g. highlight the back-to-board button.)
