...
* Maybe add a step-back button to the control panel at the bottom. Requires keeping previous state in memory, or several so we can step back multiple ticks.
* If we do the "asleep vs active regions" optimization below, or if we store previous state for step-back, then as a follow-up: when testing a solution, check for loops (no active regions, or previous state equals current state) and end the test early.

## New puzzle types

* Add support for a new puzzle type, where the player starts with a machine that doesn't work. They have to modify as few tiles as possible to make it work. Same scoring rules but we only count modified tiles. Add some way to view what tiles have been modified - maybe color grid lines yellow if their contained cell is modified.
* After the last set of puzzles is unlocked, also unlock a "full toolbelt" equivalent of every puzzle - a variant where all components are available, with the same list of prices for each. This adds content, lets players compete on more histograms.
* Add a variant of puzzle pricing, where instead of a price per block, you have a fixed number of each block, shown on the palette. Allow placing more than that, but show warning in bottom-left if puzzle exceeds allowed amount, and don't register success or entirely block testing.
...
* For community puzzles, organize them by their set of allowed components. Unlock each community puzzle group after the earliest progression point where all of those components have appeared in that group or an earlier group.
...
* Add beam-emitter blocks. They cast a beam of light forward. Beams travel through empty cells, glass, and gemstones. Beams travel in straight rows or columns through the centers of cells. Beams travel instantly in a single tick, not 1 cell per tick. Beams are not blocks, they're tracked separately. Beams can be white, blue, or red. The emitter creates a white beam; a -1 or +1 rear charge changes the beam's color to red/blue. The charge sensor block's line of sight is stopped by both solid blocks and beams, i.e. if there's a beam in front then it reads that beam's color (or zero on white) instead of what's behind it.
	* As a follow-up, ban beams from crossing. If there is a horizontal beam, and then a vertical beam activates that would shine through the horizontal beam, the vertical beam stops where it meets the horizontal one, while the horizontal one continues to pass through all the way. If the horizontal beam is briefly interrupted, the vertical beam continues, so when the horizontal beam resumes, it's blocked by the vertical beam. If both beams activate simultaneously, they're both stopped at the intersection. This all implies we can't deduce beams as a pure function of the blocks on the board - they're stateful, depending on the previous state of the beams. Ideally we also want this to work when the beam emitters move, e.g. if the horizontal beam is currently stopped by the vertical beam which passes through, then a horizontal move in the vertical beam's emitter shouldn't allow the horizontal beam's emitter to dominate.
	* As a follow-up, add a beam comparer. It looks ahead in its forward direction, checks the first beam, and then compares the block under that beam against the block behind the comparer. It emits +1 to its left/right sides if these blocks match, or 0 otherwise. (This allows using a beam emitter and comparer together to non-destructively read the blocks inside a specific area, as long as they're glass or gemstones. So we can construct in-world ROMs without the ROM block.)
	* As a follow-up, add a beam swapper. It swaps the block behind it with the first block ahead that's overlapping a beam.
...
* Modify confirmation dialogs we currently have - on resizing in the sandbox, cropping in sandbox, and deleting sandboxes and puzzle solutions. Maybe replace with having to hold down the button, or click twice on the same button to confirm.

## Puzzle briefing screen

* Add text and art in the puzzle briefing - write a story to explain why the player is solving this puzzle.

# Visuals

* Add background art for puzzles. Maybe caves, multiple layers, with parallax as player pans.

## Specific block appearance changes

* Rework magnet artwork alongside the planned electromagnet mechanics.
* For the rotator component, we should modify rendering to make behavior more obvious. Maybe draw as a welded block with only around a third of the width, on welded side, and then draw the rotator arm separately. Also mark red/blue on the sides of the base to show which charge rotates in which direction.
...
* Add hexagonal grids for some puzzles.
* Add a system of mechanical devices, like our current ternary circuit system (conduits, inverter, etc.) but with different visuals and different mechanics. We're restricted to motion that's legible in our 2D side view. Add chain drives that can rotate clockwise (+1), counterclockwise (-1), or stay still. Add gears that rotate in the inverse direction from a neighbor, and equivalents to other rune/circuit components.
* Blocks that set specific rules, e.g. what can be smelted to what, or what the assembler recipes are. Allows puzzles in the vein of Baba Is You, and more freedom in puzzle design.
* Add elf archers with some simple behavior. Add arrows that they can shoot, which arc up for 2 tiles diagonally, then travel to the side and destroy the first block they hit.
* Add a solitaire-esque minigame. Maybe a solitaire variant where you move polyominoes of stone/dirt around to uncover gems, working in limited space. Or maybe an existing puzzle type like Sto-Stone.
