...
| Furnaces | Transform sand to glass in four active ticks or ore to iron in six. Charged rear input pauses; target-ID changes reset progress. Side outputs are positive while baking. Recipes live in `furnace.ts`. |
...
* Sequence checkers ignore rear input until its first nonzero value, then compare one row-major expected value per tick. They latch failure at the mismatched cursor or success past the final value. Reconfiguration rewinds cursor/verdict.
