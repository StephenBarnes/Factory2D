...
| Drills | Destroy the immediate front tile and its welds, not its whole body. Previous-tick shared side -1 disables; 0/+1 enables. Collect stable target IDs before circuits, then commit after production/consumption and before motion, skipping removed/replaced targets. Multiple drills claiming one tile jam; mutually facing drills destroy each other simultaneously. Front welds are forbidden. Tile metadata's optional `indestructible` flag protects platforms from drilling; other kinds remain destructible. Protection is checked on observation and commit, and does not prevent editor deletion or delivery consumption. |
