...
| Furnaces | Transform sand to glass in four active ticks or ore to iron in six. Shared side -1 pauses using start-of-tick charge, preserving progress; target-ID changes reset progress. The isolated rear output is +1 on each active baking tick, including completion, and 0 while paused or without a recipe. Welded side ports link adjacent furnaces into one control network. Recipes live in `furnace.ts`; optional `weldTo` kinds select orthogonal neighbors before cooking, and eligible welds commit after all transformations, before motion. Glassmaking joins existing glass only: simultaneously cooked sand does not join, and later ticks do not add welds. |
