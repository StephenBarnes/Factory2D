...
* A 20x14 editable Canvas 2D grid with procedural sand, falling stone, magnetic metal, directional magnets, circuit conduits, and sensor, inverter, combiner, and rectifier runes.
...
* Circuit-capable tiles render charge-colored traces only across welded circuit connections. Conduits have a dark center socket; sensor runes isolate their pointed side from circuit links and color their arrow by sensed output independently from the connected network's charge; directional gates render isolated input and output segments in each port's own charge color, with distinct inverter, combiner, and rectifier rune symbols.
