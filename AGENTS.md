...
* `e2e/app-lifecycle.spec.ts` — Playwright lifecycle coverage for routing, edge-panel and canvas geometry, solution persistence/actions and successful scores, visible and fast-forwarded puzzle tests, paused failure state, test-case selection, export dropup availability and downloads, and narrow-screen overflow.
