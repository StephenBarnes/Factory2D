...
* Circuit-capable tiles render charge-colored traces only across welded circuit connections. Conduits have a compact charge-colored center socket; sensor runes isolate their pointed side from circuit links and color their arrow by sensed output independently from the connected network's charge; directional gates render isolated input and output segments in each port's own charge color, with distinct inverter, combiner, rectifier, multiplier, and subtractor rune symbols.
