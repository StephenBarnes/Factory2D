...
* `src/game/puzzle-test-runner.ts` — Isolated sequential puzzle-case execution, editable-design application, terminal outcome capture, and all-case success reporting.
...
* `src/ui/puzzle-test-report.ts` — Accessible per-case success/failure modal rendering and report actions.
...
* `tests/puzzle-test-runner.test.ts` — Editable-design transfer, isolated sequential execution, all-case success, loss, and cycle-limit tests.
...
* `SavedSolutionController` owns saved-solution records, per-puzzle selection, and the dirty set. Editable puzzle changes copy into the active session's `baseline` and mark that solution dirty. Every navigation transition persists the dirty baseline before changing sessions; `pagehide` is the final persistence boundary. Creation, duplication, deletion, completed pointer gestures, and explicit clearing persist immediately.
...
* Puzzle completion is recorded only after isolated execution of every declared test case succeeds. Individual workshop steps never change puzzle progress. A failed case, a cycle limit, or an already-recorded success makes no progress change. Newly recorded progress changes menu and direct-route prerequisite checks without replacing the active workshop.

## Current TODOs

Game/puzzle flow:
* Modify the export button. Instead of immediately downloading and copying to clipboard, show a small dropup menu. Options: "download scene file", "copy scene to clipboard", "download image" (replacing the current separate "image" button), "download puzzle file", and "share puzzle". The last 2 will be sandbox-only and are for authoring puzzles and later the community puzzle sharing system. The download-puzzle option should use the same format as the puzzles in `src/game/puzzles/`, with placeholders for some fields (like name, description, prerequisites, player-modifiable region, and test cases). Later we'll add ways to set the modifiable region and the test cases in the sandbox. The share-puzzle button should be disabled for now since the backend database isn't implemented yet.
