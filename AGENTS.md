...
* `src/game/puzzles.ts` — Eager shipped-puzzle registry, cross-file prerequisite validation and unlock checks, and fresh sandbox and puzzle world factories.
* `src/game/puzzle-format.ts` — Versioned strict puzzle JSON parser for metadata, feature labels, prerequisites, priced components, editable regions, and embedded board state.
...
* `tests/puzzle-format.test.ts` — Production shipped-puzzle loading, strict field validation, source-specific errors, registry prerequisites, and independent initial-world tests.
