...
Game/puzzle flow:
* Require at least one victory block to be present in every puzzle, in every test case. Reject puzzles without this. Modify our current 3 placeholder puzzles to add a victory block anywhere in the puzzle.
* Add a button to test the current solution - runs all test cases in series, with some time limit (defined per puzzle or test case, with a hard max limit), then checks if all resulted in victory, and displays a report with the puzzle's success/failure, with buttons to continue editing or go back to puzzle info screen. As a follow-up, also compute and display score: price, cycles, footprint.
* Modify the export button. Instead of immediately downloading and copying to clipboard, show a small menu, like a dropdown menu except it rises up instead of dropping down. Options: "Download scene file", "Copy scene to clipboard", "Download puzzle file", and "Share puzzle". The last 2 will be sandbox-only and are for user-created puzzles. The download-puzzle option should use the same format as the puzzles in `src/game/puzzles/`, with placeholders for some fields (like name, description, prerequisites, and player-modifiable region). The share-puzzle button should be disabled for now since the backend database isn't implemented yet.
