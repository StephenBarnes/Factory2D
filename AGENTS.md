...
* A 20x14 editable Canvas 2D grid with procedural sand, falling stone, glass, iron ore, magnetic iron, directional magnets and furnaces, clockwise/counterclockwise conveyor belts, circuit conduits and wire crossings, and occupancy sensor, charge sensor, inverter, combiner, rectifier, multiplier, subtractor, and selector runes.
...
* Circuit-capable tiles render charge-colored traces only across welded circuit connections. Conduits have a compact charge-colored center socket; conveyor belts have charge-colored sockets and perimeter dashes that animate in the driven direction; wire crossings render independently colored horizontal and vertical traces with a visible bridge; occupancy sensor runes isolate their pointed side from circuit links and color their arrow by sensed output independently from the connected network's charge; directional gates and furnaces render isolated input and output segments in each port's own charge color, with distinct charge sensor, inverter, combiner, rectifier, multiplier, subtractor, selector, and furnace symbols.
* A typed-array world with stable tile IDs, per-tile orientation and signed-ternary charge, independent horizontal and vertical wire-crossing charge, furnace bake progress and target identities, edge weld storage, and allocation-free per-tick movement, circuit-network, and furnace buffers.
* Deterministic driven movement resolves conveyor force sums, opposite belt reactions, complete pushing chains, fixed-body blocking, and equal-priority destination jamming before passive gravity. Straight-down gravity, complete downward body dependencies, parity-selected diagonal sand gravity, direct-fall priority, and reciprocal magnetic constraints remain deterministic.
* Deterministic circuit resolution rebuilds welded networks from the start-of-tick state, sums their drivers, takes the sign, and commits the result before movement. Wire crossings resolve horizontal and vertical axes as independent networks. Directional occupancy sensor runes contribute +1 when their pointed neighboring cell is occupied. Gates keep every input and output network isolated and drive tick t+1 from charges observed at tick t: charge sensors read their pointed adjacent tile without requiring a weld and copy its charge to the other three sides, inverters negate the sum of up to three inputs, combiners resolve the sign of up to three inputs, rectifiers pass positive sums from up to three inputs, multipliers multiply up to three circuit-connected inputs, subtractors drive `sign(back - left - right)`, and selectors use the rear input to choose between left and right. Directional furnaces bake sand into glass in four active ticks or iron ore into iron in six active ticks, pause while their isolated rear input is charged, reset when the target identity changes, and emit +1 from both side outputs while actively baking.
...
* `src/simulation/board-export.ts` — Deterministic compact ASCII tile-and-weld-grid JSON serialization and strict validation/deserialization for sharing board and furnace state.
* `src/simulation/world.ts` — Typed-array tile, orientation, charge, wire-crossing axis charge, furnace progress/target, and weld storage; stable IDs; render revisions; snapshots; editing; transformations; and body movement commits.
* `src/simulation/simulation.ts` — Allocation-free circuit-network, delayed gate, and furnace resolution; welded and magnetically constrained body collection; conveyor force/reaction and pushing-chain resolution; gravity intent selection; conflict resolution; and tick advancement.
...
* `tests/circuit.test.ts` — Instant welded-network propagation, wire-crossing axis isolation, occupancy and charge sensor directionality, isolated gate networks and delay, combiner, multiplier, subtractor, and selector truth tables, disconnection, and moving-charge tests.
* `tests/furnace.test.ts` — Furnace recipe timing, target identity and movement, circuit control/output, and snapshot tests.
* `tests/conveyor.test.ts` — Conveyor circuit direction, all-side force, reaction, weld isolation, push-chain, metadata, and board-format tests.
...
Game flow:
* Implement a main menu. For now, continue booting straight to the sandbox for faster testing during development, but add a button to go to a main menu screen, with a button for the sandbox mode and disabled button for puzzles.
* Implement a system for defining puzzles - similar to the current export format with some extra fields, such as a list of components that are placeable with their prices in talents.

Refactoring:
* After editing code files, if any refactoring seems necessary, add items here.

Physics bugs:
* If we build a hollow rectangle of platform blocks, and then beneath the ceiling we place a sensor (pointed upward) welded to a conveyor belt block to its right, this 2-block machine will travel leftward against the ceiling and then stop in the corner. The bug is that this assembly should fall, ideally immediately since there's nothing below it to support it, but it should especially fall once it reaches the top-left corner since it's no longer even moving left. Probably caused by the leftward force from the conveyor belt against the ceiling overriding gravity. We don't want to add a special case for this; how can we fix this class of bugs in general? Maybe we should make gravity overpower conveyor belt forces? But still allow magnet forces to overpower gravity. So the order is `conveyor < gravity < magnet`. Or maybe it would be better to allow simultaneous vertical and horizontal forces, resulting in a diagonal movement, if that won't collide, else nullify either horizontal or vertical movement (preferring horizontal, say).
* Relatedly, a vertical column of conveyor belts can currently lift single blocks upward, which seems unrealistic.
* A different conveyor belt bug: clockwise and counterclockwise conveyors do not behave symmetrically. With a column of conveyors going clockwise, a loose block on the left is pushed upwards, while a loose block on the right side is fixed in place immune to gravity (a bug). With a column of counterclockwise conveyors, a block on the left side falls (which is expected) while a block on the right side is fixed in place.
