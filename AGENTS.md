...
* Deterministic circuit resolution rebuilds welded networks from the start-of-tick state, sums their drivers, takes the sign, and commits the result before movement. Directional sensor runes contribute +1 when their pointed neighboring cell is occupied. Gates keep every input and output network isolated and drive tick t+1 from charges observed at tick t: inverters negate the sum of up to three inputs, combiners resolve the sign of up to three inputs, rectifiers pass positive sums from up to three inputs, multipliers multiply their left and right inputs, and subtractors drive the sign of their left input minus their right input.
...
* Modify the multiplier to multiply up to 3 inputs, but only consider those of its 3 inputs that are circuit-connected to another tile. For each input side that's welded to something that supports circuit connections, include that as a factor, but don't include sides that are unwelded, or welded to a block like stone that doesn't support circuit links - because otherwise you'd always have to weld all 3 input sides or the output would always be zero.
