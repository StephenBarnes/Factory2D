...
* Circuit-capable tiles render charge-colored traces only across welded circuit connections. Conduits have a dark center socket; sensor runes show their sensing direction and resolved charge; inverter, diode, and sum runes show distinct directional symbols and resolved output charge.
...
* Minor visual bug: A sensor rune connected to a circuit network should show the circuit network's overall color on wires, but the arrow should show what the sensor is trying to output rather than the overall value of the network. If we have a circuit network that's forced to value zero, and a connected sensor is trying to write +1 because it senses a block, the arrow should show as blue but any other wires drawn on the rune block should show as the network's value. Similarly the arrow should show as dark purple when nothing is sensed, even if other wires have nonzero charge.
* The sensor rune should not allow circuit connections with the block in the direction it's facing. It can be welded on that side, but should not allow circuit links.
* Unify the sum and diode runes into a single "combiner rune". The combiner rune should have one output, and up to 3 inputs. The inputs' networks should not be joined to each other by the combiner. Use a directional symbol, so output direction is obvious. The up to 3 inputs and output should not visually connect with each other in the center of the block, but rather terminate a short distance inside the block, and be colored according to their individual charges.
* The inverter rune and diode rune should not visually connect input and output - both lines should terminate a short distance inside the cell, and each segment should be colored according to that side's charge.
* Figure out which ternary logic gates are not special cases of small numbers of inverters and combiners.
