...
Game/puzzle flow:
* Add a button to test the current solution - runs each test case in series, with some cycle limit (defined per puzzle or test case, with a hard max and a default), then checks if all of them resulted in victory, and displays a report modal with the puzzle's success/failure. A solution succeeds only if all test cases result in victory. The modal should have buttons to continue editing or go back to the puzzle info screen. The "test" button should replace the current "run" button on all puzzles; the sandbox can keep "run", and should not show this report modal.
* After the solution has been tested, if it succeeds, compute and display scores: price, cycles until victory (averaged over test cases), footprint (size of AABB containing all nonempty blocks inside modifiable regions), and combined score (sum of cost, footprint tiles, and cycles). Later this will also have a histogram of other players' scores.
* Modify the export button. Instead of immediately downloading and copying to clipboard, show a small menu, like a dropdown menu except it rises up instead of dropping down. Options: "Download scene file", "Copy scene to clipboard", "Download image" (replacing the current separate "Image" button), "Download puzzle file", and "Share puzzle". The last 2 will be sandbox-only and are for authoring puzzles and player-created puzzles. The download-puzzle option should use the same format as the puzzles in `src/game/puzzles/`, with placeholders for some fields (like name, description, prerequisites, and player-modifiable region). The share-puzzle button should be disabled for now since the backend database isn't implemented yet.
