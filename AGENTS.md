...
* Bug: Hold control to weld blocks, and then LMB-drag over a row of blocks rapidly. It will weld only some of them together. It's probably only performing welding if the mouse happens to be over the weld joint at a frame boundary - but with fast mouse movement, the mouse may skip over some boundaries. So instead, if the mouse moves fast in one frame, we should find all the welds it would have done on the line between the previous and new mouse positions. A variant of this bug also occurs when using LMB/RMB drag to place/remove tiles - it doesn't seem to skip any intermediate tiles, but it does skip tiles close to the boundary of the grid, when the rapidly-moving mouse leaves the grid.
