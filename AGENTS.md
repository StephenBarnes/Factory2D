...
* A 20x14 editable Canvas 2D grid with procedural sand, falling stone, magnetic metal, directional magnets, circuit conduits, and occupancy sensor, charge sensor, inverter, combiner, rectifier, multiplier, subtractor, and selector runes.
...
* Circuit-capable tiles render charge-colored traces only across welded circuit connections. Conduits have a compact charge-colored center socket; occupancy sensor runes isolate their pointed side from circuit links and color their arrow by sensed output independently from the connected network's charge; directional gates render isolated input and output segments in each port's own charge color, with distinct charge sensor, inverter, combiner, rectifier, multiplier, subtractor, and selector rune symbols.
...
* `tests/circuit.test.ts` — Instant welded-network propagation, occupancy and charge sensor directionality, isolated gate networks and delay, combiner, multiplier, subtractor, and selector truth tables, disconnection, and moving-charge tests.
...
New circuit components - Add more signed-ternary circuit components beyond the current occupancy sensor, charge sensor, inverter, combiner, rectifier, multiplier, subtractor, and selector runes. Gates should keep input and output networks separate and drive tick t+1 from values observed at tick t. Add small tests for these.
