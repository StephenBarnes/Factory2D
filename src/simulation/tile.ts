...
export const TILE_DEFINITIONS: Readonly<Record<TileKind, TileDefinition>> = {
  ...
  [TileKind.Furnace]: {
    ...
    palette: {
      ...
      extendedDescription: ["Wood becomes fire in two active ticks; sand becomes glass in four; iron ore becomes iron in six. Copper ore becomes copper in six active ticks while wood touches an orthogonal side of the ore; the wood is not consumed, and removing it pauses progress. A -1 on the shared side circuit also pauses progress; 0 or +1 allows cooking.", "Igniting wood removes its welds; the new fire spreads to adjacent wood on the following tick, then disappears. Freshly cooked glass welds to adjacent existing glass, but not to sand cooked in the same tick. Metal smelting adds no welds.", "The isolated rear output is +1 on every active baking tick, including completion. Moving a different target in front resets progress."],
    },
    affectedByGravity: true,
    slidesDiagonally: false,
    weldableSides: WeldSide.All,
