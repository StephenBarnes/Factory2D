...
export const TILE_DEFINITIONS: Readonly<Record<TileKind, TileDefinition>> = {
  ...
  [TileKind.Wood]: {
    ...
    palette: {
      ...
      extendedDescription: ["Falls one cell per tick when unsupported and can be welded into a rigid body. This material is nonmagnetic and does not conduct circuit charge.", "Orthogonally adjacent fire consumes wood, replacing it with unweldable fire and removing its welds. A furnace facing wood also turns it into fire after two active ticks. Wood touching copper ore fuels its smelting and becomes fire when the copper finishes."],
    },
    affectedByGravity: true,
    slidesDiagonally: false,
    weldableSides: WeldSide.All,
  ...
  [TileKind.Furnace]: {
    ...
    palette: {
      ...
      extendedDescription: ["Wood becomes fire in two active ticks; sand becomes glass in four; iron ore becomes iron in six. Copper ore becomes copper in six active ticks while wood touches an orthogonal side of the ore; all touching wood becomes fire on completion. Removing the wood beforehand pauses progress. A -1 on the shared side circuit also pauses progress; 0 or +1 allows cooking.", "Igniting wood removes its welds; the new fire spreads to adjacent wood on the following tick, then disappears. Freshly cooked glass welds to adjacent glass, including sand cooked in the same tick. Metal smelting adds no welds.", "The isolated rear output is +1 on every active baking tick, including completion. Moving a different target in front resets progress."],
    },
    affectedByGravity: true,
    slidesDiagonally: false,
    weldableSides: WeldSide.All,
